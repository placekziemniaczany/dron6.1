...
unsigned int APIGnuPlot3D::add2shape_list(const list<list<Point3D> > & _list, const string & color) {
  shapes.insert(std::pair<unsigned int,shape_and_color>(last_shape,std::make_pair(_list,color)));
  return last_shape++;
}
...
unsigned int APIGnuPlot3D::draw_line(const Point3D & point1, const Point3D & point2, const string & color ) {
  unsigned int tmp = add2shape_list(list<list<Point3D> > {{point1,point2}},color);
...
unsigned int APIGnuPlot3D::draw_polygonal_chain(const vector<Point3D> & points, const string & color ) {
  unsigned int tmp = add2shape_list(list<list<Point3D> > {list<Point3D>(begin(points),end(points))},color);
  if (refresh_rate_ms == 0) {
    draw_all_shapes();
  }
  return tmp;
...
unsigned int APIGnuPlot3D::draw_polyhedron(const vector<vector<Point3D> > & points_map, const string & color ) {
...
unsigned int APIGnuPlot3D::draw_surface(const vector<vector<Point3D> > & points_map, const string & color ) {
  ...
  unsigned int tmp = add2shape_list(ltmp,color);
  if (refresh_rate_ms == 0) {
    draw_all_shapes();
  }
  return tmp;
...
void APIGnuPlot3D::erase_shape(unsigned int id) {
  shapes.erase(id);
  if (refresh_rate_ms == 0) {
    draw_all_shapes();
  }
...
void APIGnuPlot3D::change_shape_color(unsigned int id, const string & color) {
  if (shapes.count(id)) {
