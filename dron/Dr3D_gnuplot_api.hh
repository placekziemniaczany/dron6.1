...
class APIGnuPlot3D : public drawNS::Draw3DAPI {
   ...
   */
  std::map<unsigned int,shape_and_color> shapes;
  void send2gnuplot(const std::string & command_arg);
  void replot_loop();
  unsigned int add2shape_list(const std::list<std::list<Point3D> > & _list, const std::string & color);
...
public:
  ...
  unsigned int draw_line(const Point3D & point1, const Point3D & point2, const std::string & color = "black") override;
  unsigned int draw_polygonal_chain(const std::vector<Point3D> & points, const std::string & color = "black") override;
  unsigned int draw_polyhedron(const std::vector<std::vector<Point3D> > & points_map, const std::string & color = "black") override;
  unsigned int draw_surface(const std::vector<std::vector<Point3D> > & points_map, const std::string & color = "black") override;
  void erase_shape(unsigned int id) override;
  void change_shape_color(unsigned int id, const std::string & color) override;
